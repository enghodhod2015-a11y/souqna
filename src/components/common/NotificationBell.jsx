...
export const NotificationBell = () => {
  ...
  const getNotificationPath = (notif) => {
    if (notif.related_id) {
      return `/product/${notif.related_id}`
    }
    return null
  }
  ...
  const markAsReadAndNavigate = async (notif) => {
    try {
      await markNotificationAsRead(notif.id).catch(err => console.error(err))
      ...
      if (path) {
        ...
        // عرض الإشعار الإداري في مودال منفصل
  ...
  return (
    <>
      ...
      {/* مودال عرض الإشعار الإداري */}
      {selectedNotification && (
        <Modal onClose={() => setSelectedNotification(null)} title={selectedNotification.title}>
          <div className="p-2">
            <p className="text-text-secondary whitespace-pre-wrap">{selectedNotification.message}</p>
            <div className="mt-4 text-left text-xs text-text-secondary">
              {new Date(selectedNotification.created_at).toLocaleString('ar')}
            </div>
          </div>
        </Modal>
      )}
