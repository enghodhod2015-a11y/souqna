...
export const Header = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      ...
      // لا نغلق قائمة الموبايل هنا لتجنب التعارض
    }
  ...
  // روابط عامة (يتم إعادة حسابه عند تغير user/profile)
  const navLinks = useMemo(() => {
      ...
      <>
        ...
        {user ? (
          ...
          <div className="flex flex-col gap-3">
            <Link to="/login" className="bg-gold text-primary-blue px-4 py-2 rounded-lg font-bold hover:bg-gold/90 transition text-center">
              تسجيل دخول
            </Link>
            <Link to="/register" className="border border-gold px-4 py-2 rounded-lg text-gold hover:bg-gold/10 transition text-center">
              إنشاء حساب
            </Link>
          </div>
        )}
    ...
    return links
  }, [user, profile, isSeller, isAdmin, unreadCount, logout, navigate])
    ...
    <header className="bg-header-blue border-b-2 border-gold py-4 px-6 md:px-12 relative z-40">
      ...
      {isMobileMenuOpen && (
        <>
          ...
          <div 
            ref={mobileMenuRef}
            className="fixed top-16 left-0 right-0 bg-primary-card border-b border-gold/30 shadow-2xl z-50 p-4 flex flex-col gap-3 max-h-[80vh] overflow-y-auto rounded-b-2xl"
            style={{ backgroundColor: '#0a2a4a' }}
          ...
          >
            ...
            <div className="flex justify-end mb-2">
              <button
                onClick={() => setIsMobileMenuOpen(false)}
                className="text-gold p-1 rounded-full hover:bg-gold/20"
              >
                <X size={20} />
              </button>
            </div>
