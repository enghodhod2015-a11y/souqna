...
export default function AdminDashboardPage() {
  ...
  const handleAddTransfer = async () => {
    ...
    try {
      ...
      // 2. إدراج السجل عبر RPC (لتجاوز RLS)
      const { error: insertError } = await supabase.rpc('insert_seller_transfer', {
        seller_id_param: selectedSeller.id,
        amount_param: parseFloat(transferAmount),
        receipt_image_param: publicUrl,
        note_param: transferNote || ''
      })
      ...
      {activeMainTab === 'dashboard' && (
        <div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            ...
            <div className="bg-primary-card p-4 rounded-2xl border border-gold/30"><Wallet className="text-gold mb-2" size={32} /><p className="text-text-secondary text-sm">سحوبات معلقة</p><p className="text-2xl font-bold">0</p></div>
          </div>
          ...
          {activeSubTab === 'sellers' && (
            <div>
              <div className="mb-4">
                ...
                <Select value={selectedSeller?.id || ''} onChange={(e) => { const seller = sellerUsers.find(u => u.id === e.target.value); setSelectedSeller(seller); setSellerDetailTab('profile'); setSellerFilterId(null); }} className="w-full md:w-1/2 bg-white text-gray-900 border border-gold/30">
                ...
                <div className="bg-primary-card rounded-2xl border border-gold/30 p-4 mt-4">
                  ...
                  {sellerDetailTab === 'finance' && (
                    <>
                      ...
                      <div className="mt-4">
                        <Button variant="secondary" onClick={() => navigate(`/admin/seller-transfers/${selectedSeller.id}`)}>
                          عرض جميع الإيصالات
                        </Button>
                      </div>
                    </>
