...
export default function NotificationsPage() {
  ...
  const handleNotificationClick = async (notif) => {
    ...
    if (notif.type === 'message' && notif.related_id) {
      ...
      if (notif.related_id) {
        navigate(`/orders`)  // أو يمكن توجيهه إلى صفحة الطلب المحددة
      } else {
        navigate('/orders')
      }
    }
    else if (notif.related_id && (notif.type === 'info')) {
      // إشعار إداري مرتبط بمنتج
      navigate(`/product/${notif.related_id}`)
    }
    else {
      // إشعار إداري بدون related_id – عرض في toast
      toast.custom((t) => (
